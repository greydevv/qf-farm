...
export default function ShowcaseCard({ img, headText, text, moreText, reverse }) {
    return (
        <div className={ 'cursor-pointer grid w-full showcase_card' + (reverse ? '-reversed' : '')}>
            <div className='showcase_card__img w-full rounded-lg overflow-clip overflow-hidden'>
                <img src={ img } className='overflow-clip overflow-hidden object-cover w-screen min-w-full min-h-full' />
            </div>
