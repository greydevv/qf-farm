...
function Package({ headerText, bodyText, disclaimer, price, rate }) {

    const mailTo = `mailto:quillfeatherfarm@gmail.com?subject=${ headerText }: Training Package Sign Up`

    return (
        <div className='rounded-sm bg-qf-brown px-6 pt-8 pb-4'>
...
export function PackageList() {
    const [activeCategory, setActiveCategory] = useState(Object.keys(trainingPackages)[0])
    const [packages, setPackages] = useState(trainingPackages[activeCategory].packages)

    const changeCategory = (category) => {
        setActiveCategory(category)
        setPackages(trainingPackages[category].packages)
    }
